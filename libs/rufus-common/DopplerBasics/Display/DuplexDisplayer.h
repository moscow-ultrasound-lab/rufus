...
class	DuplexDisplayer 
	...
	public:
		DuplexDisplayer(const S500_CFMFrameSet &in_fs, const RealFunctionMD_F32 &in_offsets, const RealFunctionMD_F32 &in_mask_frames, const bool in_flag_cfm, string in_title);
			
		const S500_CFMFrameSet &fs;
		const RealFunctionMD_F32 &cfm_frames;
		const RealFunctionMD_F32 &mask_frames;
		const bool &cover_full_frame;
		const string title;
