...
#include <DopplerBasics/Unknown/CalcTrueFalseDetectionRates.h>
...
{
	DuplexDisplayer	dd(frames, phase_frames_normalized, mask_frames, false, "Duplex display");
	...
	enum
	{
		...
		display_std,
		display_amplitude,
		display_true_and_false_detection_rates,
		display_additionals,
	...
	while(true)
	{
		option = GetButtonDecision("Choose data to display",
		{
			...
			MakeButton("True and false detection rates",	display_true_and_false_detection_rates),
			MakeButton("Additionals",	display_additionals),
		...
		try
		{
			switch(option)
			{
				...
				case display_true_and_false_detection_rates:
					DisplayDopplerDataTrueFalseDetectionRates(cfm_phase_dispersion, correlation_frames, correlation_re_im_frames, std_frames, amplitude_frames, frames);
					break;

				case display_additionals:
