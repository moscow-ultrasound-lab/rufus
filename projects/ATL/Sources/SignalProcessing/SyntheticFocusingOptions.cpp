...
/*
int run_rufus_sa(int argc, char** argv)
{
	XRAD_USING

		try {
		// Создание объекта SyntheticFocusingOptions запускает диалог выбора источника
		SyntheticFocusingOptions options;

		// Здесь можно добавить код для отображения результата,
		// если это необходимо. Сейчас конструктор сам делает всю работу.

		ShowText("rufus-sa completed", "Processing finished.");
		return 0;
	}
	catch (canceled_operation&) {
		ShowText("Cancelled", "Operation cancelled by user.");
		return 0;
	}
		...
		ShowString("An exception occurred:", ex.what());
		return 1;
	}
	catch (...) {
