...
public:
    ...
    complex(const double &_x):re(_x),im(0.0){};
    complex(const double &_x, const double &_y):re(_x),im(_y){};
    complex(const complex &z):re(z.re),im(z.im){};

    complex& operator= (const double& v){ re  = v; im = 0.0; return *this; };
    complex& operator+=(const double& v){ re += v;          return *this; };
    complex& operator-=(const double& v){ re -= v;          return *this; };
    complex& operator*=(const double& v){ re *= v; im *= v;  return *this; };
    complex& operator/=(const double& v){ re /= v; im /= v;  return *this; };

    complex& operator= (const complex& z){ re  = z.re; im  = z.im; return *this; };
    complex& operator+=(const complex& z){ re += z.re; im += z.im; return *this; };
    complex& operator-=(const complex& z){ re -= z.re; im -= z.im; return *this; };
    complex& operator*=(const complex& z){ double t = re*z.re-im*z.im; im = re*z.im+im*z.re; re = t; return *this; };
    complex& operator/=(const complex& z)
    {
        ap::complex result;
        double e;
        double f;
        if( std::fabs(z.im)<std::fabs(z.re) )
        {
            e = z.im/z.re;
            f = z.re+z.im*e;
            result.re = (z.re+z.im*e)/f;
            result.im = (z.im-z.re*e)/f;
        }
        else
        {
            e = z.re/z.im;
            f = z.im+z.re*e;
            result.re = (z.im+z.re*e)/f;
            result.im = (-z.re+z.im*e)/f;
        }
        *this = result;
        return *this;
    };
...
#else
typedef xrad::complexF64 complex;

inline const double abscomplex(const complex &z){return cabs(z);}
inline const complex conj(const complex &z){return ~z;}
inline const complex csqr(const complex &z){return z*z;}
