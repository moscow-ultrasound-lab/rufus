...
{
    volatile double x1 = lhs.re;
    volatile double x2 = rhs.re;
    volatile double y1 = lhs.im;
    volatile double y2 = rhs.im;
    return x1==x2 && y1==y2;
}
...
const ap::complex ap::operator-(const ap::complex& lhs)
{ return ap::complex(-lhs.re, -lhs.im); }
...
const ap::complex ap::operator*(const ap::complex& lhs, const ap::complex& rhs)
{ return ap::complex(lhs.re*rhs.re - lhs.im*rhs.im,  lhs.re*rhs.im + lhs.im*rhs.re); }

const ap::complex ap::operator*(const ap::complex& lhs, const double& rhs)
{ return ap::complex(lhs.re*rhs,  lhs.im*rhs); }

const ap::complex ap::operator*(const double& lhs, const ap::complex& rhs)
{ return ap::complex(lhs*rhs.re,  lhs*rhs.im); }
...
const ap::complex ap::operator/(const ap::complex& lhs, const ap::complex& rhs)
{
    ...
    double e;
    double f;
    if( std::fabs(rhs.im)<std::fabs(rhs.re) )
    {
        e = rhs.im/rhs.re;
        f = rhs.re+rhs.im*e;
        result.re = (lhs.re+lhs.im*e)/f;
        result.im = (lhs.im-lhs.re*e)/f;
    }
    else
    {
        e = rhs.re/rhs.im;
        f = rhs.im+rhs.re*e;
        result.re = (lhs.im+lhs.re*e)/f;
        result.im = (-lhs.re+lhs.im*e)/f;
    }
    return result;
}
...
{ return ap::complex(z.re, -z.im); }

const ap::complex ap::csqr(const ap::complex &z)
{ return ap::complex(z.re*z.re-z.im*z.im, 2*z.re*z.im); }
...
void ap::vmove(ap::complex *vdst, int stride_dst, const ap::complex* vsrc, int stride_src, const char *conj_src, int n, double alpha)
{
    ...
    if( stride_dst!=1 || stride_src!=1 )
    {
        ...
        if( bconj )
        {
            for(i=0; i<n; i++, vdst+=stride_dst, vsrc+=stride_src)
            {
                vdst->re =  alpha*vsrc->re;
                vdst->im = -alpha*vsrc->im;
            }
        }
        else
        {
...
void ap::vmove(ap::complex *vdst, int stride_dst, const ap::complex* vsrc, int stride_src, const char *conj_src, int n, ap::complex alpha)
{
    ...
    if( stride_dst!=1 || stride_src!=1 )
    {
        ...
        if( bconj )
        {
            double ax = alpha.re, ay = alpha.im;
            for(i=0; i<n; i++, vdst+=stride_dst, vsrc+=stride_src)
            {
                vdst->re =  ax*vsrc->re+ay*vsrc->im;
                vdst->im = -ax*vsrc->im+ay*vsrc->re;
            }
        }
        else
        {
...
void ap::vmul(ap::complex *vdst, int stride_dst, int n, ap::complex alpha)
{
    ...
    if( stride_dst!=1 )
    {
        ...
        for(i=0; i<n; i++, vdst+=stride_dst)
        {
            double  dstx = vdst->re, dsty = vdst->im;
            vdst->re = ax*dstx-ay*dsty;
            vdst->im = ax*dsty+ay*dstx;
        }
    }
    else
    {
...
{ return int(x>0 ? std::floor(x) : ceil(x)); }
