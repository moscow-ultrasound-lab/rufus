...
// инициализация
//
DuplexDisplayer::DuplexDisplayer(const S500_CFMFrameSet &in_fs, const RealFunctionMD_F32 &in_offsets, const RealFunctionMD_F32 &in_mask_frames, bool in_flag_cover_full_frame, string in_title):
	fs(in_fs),
	cfm_frames(in_offsets),
	mask_frames(in_mask_frames),
	cover_full_frame(in_flag_cover_full_frame),
	title(in_title)
{
...
template<class ARR>
ColorImageF32	DuplexDisplayer::DrawDuplex(const ARR &mask_cfm, const ARR &slice_cfm,GradientPalette &palette,ColorScanConverter &SC,bool just_frame)
{
...
void	DuplexDisplayer::DisplayDuplex()
{
	...
	bool	grid(false);
	if (CapsLock) { grid = YesOrNo("Draw grid?", SavedGUIValue(false)); }
...
//	DisplayMathFunction3D(b_frames, "b_frames", fs.sco_cfm);
//	DisplayMathFunction3D(cfm_frames, "cfm_frames", fs.sco_cfm);
//	DisplayMathFunction3D(mask_frames, "mask", fs.sco_cfm);
	...
	for(size_t frame = 0; frame < fs.n_frames; ++frame)
	{
		...
		CopyData(SC, b_frames.GetSlice(index), db_compress_functor);  //логарифмическое сжатие
				
		SC.BuildConvertedImage();  //сканконверсия - преобразование координат с интерполяцией

		//auto mask_cfm = mask_frames.GetSlice({ frame, slice_mask(0), slice_mask(1) });
		auto mask_cfm = mask_frames.GetSlice(index);
		...
		if (cover_full_frame)
		{
			window_size_r = slice_cfm.vsize();
			cfm_density_r = double(fs.n_b_rays -  2*window_corner_r) / window_size_r;
			window_size_s = slice_cfm.hsize();
			cfm_density_s = double(fs.n_b_samples - 2*window_corner_s) / window_size_s;
		}
